...
public class ShooterGameCamera : MonoBehaviour {
	...
	void LateUpdate () {
		...
		// I removed this, so you can quick and easy add this script to your game. - ben0bi
													  //@joystick 3rd axis
		angleH += Mathf.Clamp(Input.GetAxis("joystick 3rd axis") /* + Input.GetAxis("Horizontal2") */ , -1, 1) * horizontalAimingSpeed * Time.deltaTime;
													 // @joystick 4th axis
		angleV += Mathf.Clamp(Input.GetAxis("joystick 4th axis") /* + Input.GetAxis("Vertical2") */ , -1, 1) * verticalAimingSpeed * Time.deltaTime;
